...
from compressai.datasets import ImageFolder_Czigzag
...
class RateDistortionLoss(nn.Module):
    ...
    def forward(self, input, output):
        ...
        out["mse_loss"] = self.mse(input, output["x_hat"])
...
def train_one_epoch(
    ...
    for i, d in enumerate(train_dataloader):
        ...
        out_net = model(original_img,up_x4_img)

        out_criterion = criterion(original_img,out_net)
...
def parse_args(argv):
    ...
    parser.add_argument(
        "--save_path", type=str, default="./save_model/czigzag_018/", help="Where to Save model"
    )
    parser.add_argument(
    ...
    parser.add_argument("--checkpoint",
                        default="./save_model/czigzag_018/300.ckpt",  # ./save_model/czigzag_1/8.ckpt
                        type=str, help="Path to a checkpoint")
...
def main(argv):
    ...
    train_dataset = ImageFolder_Czigzag(args.dataset, split="train", transform=train_transforms)
    test_dataset = ImageFolder_Czigzag(args.dataset, split="test", transform=test_transforms)
