...
class SymmetricalTransFormer2(CompressionModel):
    def __init__(self,
        ...
        #         use_checkpoint=use_checkpoint,
        #         inverse=True)
        #     self.sigma_layers.append(layer)
    ...
    def ZigzagSplits(self,inputs,num_slices, window_size = 8, firstOrder=1):
        ...
        for i in range(max(num_slices,Hpad // window_size,Wpad // window_size)):
            ...
            for j in range(min((i+1),num_slices)*min((i+1),Hpad // window_size)*min((i+1),Wpad // window_size)):
                ...
                outOrders.append(x_slices[:,C_index,:,H_index,:,W_index,:].contiguous().un)
                if C_index + 2 > num_slices or C_index +1 > i:
                    C_index = 0
                    if H_index + 2 > Hpad // window_size or H_index+1 > i:
                        W_index = W_index + 1
    ...
    def forward(self, x):
        ...
        # means_zigzag, _ , _    = self.ZigzagSplits(latent_scales,self.num_slices,window_size)
        # scales_zigzag, _ , _   = self.ZigzagSplits(latent_means,self.num_slices,window_size)
        ...
        for slice_index, y_slice in enumerate(y_slices):
            ...
            mu1 = mu.clone()
            mu1 = mu1.permute(0, 2, 3, 1).contiguous().view(-1, Wh*Ww, 32)
            for i in range(self.num_layers):
                mu_layer = self.mu_Swin[slice_index][i]#.to(y.device)
                mu1, Wh, Ww = mu_layer(mu1, Wh, Ww)

            mu = mu + mu1.view(-1, Wh, Ww, 32).permute(0, 3, 1, 2).contiguous()

            scale1 = scale.clone()
            scale1 = scale1.permute(0, 2, 3, 1).contiguous().view(-1, Wh*Ww, 32)
            for i in range(self.num_layers):
                layer = self.sigma_Swin[slice_index][i]#.to(y.device)
                scale1, Wh, Ww = layer(scale1, Wh, Ww)
