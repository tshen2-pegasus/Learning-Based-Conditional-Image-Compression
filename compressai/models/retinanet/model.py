...
from compressai.models.retinanet.utils import BasicBlock, Bottleneck, BBoxTransform, ClipBoxes
from compressai.models.retinanet.anchors import Anchors
...
class ResNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        compressH = x1
        output_features = [x2,x3,x4]
        return compressH,output_features,classification, regression, anchors

        # if self.training:
        #     retinaLoss = self.focalLoss(classification, regression, anchors, annotations)
        #     compressH = x1
        #     output_features = [x2,x3,x4]
        #     return compressH,output_features,retinaLoss
        # else:
...
def studentresnet50(num_classes, pretrained=False, **kwargs):
    """Constructs a ResNet-50 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = studentResNet(num_classes, Bottleneck, [3, 4, 6, 3], **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet50'], model_dir='.'), strict=False)
    return model
