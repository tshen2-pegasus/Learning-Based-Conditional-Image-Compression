...
class MaskedRCNN_FasterRCNN_Iamge_Coding(CompressionModel):
    def __init__(self,
        ...
        self.seg_g_a = CRC_two_mainCNNencoder(N, M)
        ...
        self.seg_entropy_bottleneck = EntropyBottleneck(embed_dim * 4)

        self.seg_gaussian_conditional = GaussianConditional(None)

        self.image_codec = TIC_hp(input_resolution=(512,512))
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            ...
            print('Teacher Time:', teacher_end - teacher_start)
        ...
        print('Main Encoder Time:', encoder_end - encoder_start)

        hyper_encoder_start = time.time()
        z = self.h_a(y)
        hyper_encoder_end = time.time()
        print('Hyper Encoder Time:', hyper_encoder_end - hyper_encoder_start)

        hyper_entropy_start = time.time()
        _, z_likelihoods = self.entropy_bottleneck(z)
        z_offset = self.entropy_bottleneck._get_medians()
        z_tmp = z - z_offset
        z_hat = ste_round(z_tmp) + z_offset
        hyper_entropy_end = time.time()
        print('Hyper Entropy Time:', hyper_entropy_end - hyper_entropy_start)

        hyper_decoder_start = time.time()
        latent_scales = self.h_scale_s(z_hat)
        latent_means = self.h_mean_s(z_hat)
        hyper_decoder_end = time.time()
        print('Hyper Decoder Time:', hyper_decoder_end - hyper_decoder_start)
        print('Hyper Decoder Time(half):', (hyper_decoder_end - hyper_decoder_start)/2)
        ...
        # seg ---
        # ----
        seg_y = self.seg_g_a(torch.cat([x_hat, inputIMGs], dim=1))

        seg_z = self.seg_h_a(seg_y)
        _, seg_z_likelihoods = self.seg_entropy_bottleneck(seg_z)
        seg_z_offset = self.seg_entropy_bottleneck._get_medians()
        ...
        for slice_index in range(self.num_slices * num_H * num_W):
            # support_slices = (y_hat_slices if self.max_support_slices < 0 else y_hat_slices[:self.max_support_slices])
            support_slices = (seg_y_hat_slices if self.max_support_slices > slice_index else seg_y_hat_slices[
                                                                                         slice_index - self.max_support_slices:])

            support_num = self.support_num
            # meanInput = ([means_zigzag[:,slice_index:slice_index+self.max_support_slices,:,:,:]] if slice_index > slice_index else y_hat_slices[slice_index-self.max_support_slices:])
            if slice_index + support_num > self.num_slices * num_H * num_W:
                meanInput = means_zigzag[:, -support_num:, :, :, :].view(-1, 384 * support_num // self.num_slices,
                                                                         (H // number), (W // number))
            else:
                meanInput = means_zigzag[:, slice_index:slice_index + support_num, :, :, :].view(-1,
                                                                                                 384 * support_num // self.num_slices,
                                                                                                 (H // number),
                                                                                                 (W // number))
            mean_support = torch.cat([meanInput] + support_slices, dim=1)
            # print(slice_index,'support_slices',len(support_slices),mean_support.shape)
            mu = self.seg_cc_mean_transforms2[slice_index](mean_support)
            # mu = mu[:, :, :y_shape[0], :y_shape[1]]

            if slice_index + support_num > self.num_slices * num_H * num_W:
                scaleInput = scales_zigzag[:, -support_num:, :, :, :].view(-1, 384 * support_num // self.num_slices,
                                                                           (H // number), (W // number))
            else:
                scaleInput = scales_zigzag[:, slice_index:slice_index + support_num, :, :, :].view(-1,
                                                                                                   384 * support_num // self.num_slices,
                                                                                                   (H // number),
                                                                                                   (W // number))
            # scaleInput = scales_zigzag.view(-1,384*number*number,(H//number),(W//number))
            scale_support = torch.cat([scaleInput] + support_slices, dim=1)
            scale = self.seg_cc_scale_transforms2[slice_index](scale_support)
            # print(y_zigzag[:,slice_index,:,:,:].shape)
            # print(scale.shape)
            # print(mu.shape)
            _, seg_y_slice_likelihood = self.seg_gaussian_conditional(y_zigzag[:, slice_index, :, :, :], scale, mu)

            seg_y_likelihood.append(seg_y_slice_likelihood)
            seg_y_hat_slice = ste_round(seg_y_zigzag[:, slice_index, :, :, :] - mu) + mu

            lrp_support = torch.cat([mean_support, seg_y_hat_slice], dim=1)
            lrp = self.seg_lrp_transforms2[slice_index](lrp_support)
        ...
        seg_y_likelihoods = torch.cat(seg_y_likelihood, dim=1)

        seg_x_hat = self.seg_g_s(seg_y_hat) + x_hat

        image_out = self.image_codec(inputIMGs)


        return {
            "x_hat":seg_x_hat, # image_out['x_hat'] + seg_x_hat
            "compressH": x_hat,
            "likelihoods": {"y": y_likelihoods, "z": seg_y_likelihoods},
            "Student_output_features": x_hat,
            "Teacher_output_features": y_hat,
            'seg_hat':y_likelihoods,
            'image_out':image_out['y_hat']
        }
