...
class SymmetricalTransFormer3(CompressionModel):
    def __init__(self,
        ...
        for i in range(num_slices*4):
            ...
            self.mu_Swin.append(mu_layer)          

        self.sigma_Swin = nn.ModuleList()
        for i in range(num_slices):
            ...
            self.sigma_Swin.append(sigma_layer)  

        self.LRP_Swin = nn.ModuleList()
        for i in range(num_slices):
            ...
            self.LRP_Swin.append(LRP_layer)
        ...
        self.cc_mean_transforms2 = nn.ModuleList(
            nn.Sequential(
                conv(384 //self.num_slices + 384 //self.num_slices * min(i, self.max_support_slices), 224, stride=1, kernel_size=3),
                nn.GELU(),
        ...
        self.lrp_transforms2 = nn.ModuleList(
            nn.Sequential(
                conv(384 //self.num_slices + 384 //self.num_slices * min(i + 1, self.max_support_slices+1), 224, stride=1, kernel_size=3),
                nn.GELU(),
                conv(224, 176, stride=1, kernel_size=3),
                nn.GELU(),
                conv(176, 128, stride=1, kernel_size=3),
    ...
    def ZigzagSplits(self,inputs,num_slices):
        ...
        for i in range(max(num_slices,number,number)):
            C_index = 0
            H_index = W_index = 0
            for j in range(min((i+1),num_slices)*min((i+1),number)*min((i+1),number)):
    ...
    def forward(self, x):
        ...
        for slice_index in range(self.num_slices* num_H* num_W):
            ...
            mean_support = torch.cat([means_zigzag[:,slice_index,:,:,:]] + support_slices, dim=1)
            # print('support_slices',len(support_slices),mean_support.shape)
            mu = self.cc_mean_transforms2[slice_index](mean_support)
            # mu = mu[:, :, :y_shape[0], :y_shape[1]]

            scale_support = torch.cat([scales_zigzag[:,slice_index,:,:,:]] + support_slices, dim=1)
            ...
            for i in range(self.num_layers):
                mu_layer = self.mu_Swin[slice_index][i]#.to(y.device)
                mu1, _, _ = mu_layer(mu1, (H//number), (W//number))

            mu = mu + mu1.view(-1, (H//number), (W//number), 384 //self.num_slices).permute(0, 3, 1, 2).contiguous()

            # scale1 = scale.clone()
            # scale1 = scale1.permute(0, 2, 3, 1).contiguous().view(-1, window_size*window_size, 32)
            # for i in range(self.num_layers):
            #     layer = self.sigma_Swin[slice_index][i]#.to(y.device)
            #     scale1, _, _ = layer(scale1, _, _)

            # scale = scale + scale1.view(-1, Wh, Ww, 32).permute(0, 3, 1, 2).contiguous()
            # print('y_slice',y_slice.shape)
            ...
            # lrp1 = lrp.clone()
            # lrp1 = lrp1.permute(0, 2, 3, 1).contiguous().view(-1, Wh*Ww, 32)
            # for i in range(self.num_layers):
            #     layer = self.LRP_Swin[slice_index][i]#.to(y.device)
            #     lrp1, Wh, Ww = layer(lrp1, Wh, Ww)

            # lrp = lrp + lrp1.view(-1, Wh, Ww, 32).permute(0, 3, 1, 2).contiguous()
        ...
        # print(torch.cat(y_hat_slices, dim=1).shape)
