...
class Mask3dAttention(nn.Module):
    ...
    def forward(self, x ):
        ...
        # mask11 = torch.tril(mask11,diagonal = N//2 - 1) # , diagonal=-1
        ...
        mask = torch.tril(mask, diagonal=-1) # , diagonal=-1
        mask = mask.masked_fill(mask != 0, float(0.0)).masked_fill(mask == 0, float(-1000.0))
...
class NeighborSample(nn.Module):
    def __init__(self,window_size):
        ...
        self.window_size = window_size
        self.unfolder = nn.Unfold(window_size)
  
    def forward(self, inputs):
        b, c, h, w = inputs.size()
        inputs = F.pad(inputs, (self.window_size//2, 0, self.window_size//2, 0))
        t = self.unfolder(inputs) # (b, c*5*5, h*w)
        t = t.permute((0,2,1)).reshape(b*h*w, c, 5, 5)
        return t

class ClipEncoder4(CompressionModel):
    def __init__(self,
        ...
        self.cc_mean_transforms = nn.Sequential(
                conv(2*27*384//self.num_slices, 27*384//self.num_slices, stride=1, kernel_size=3),
                nn.GELU(),
                conv(27*384//self.num_slices, 15*384//self.num_slices, stride=1, kernel_size=3),
                nn.GELU(),
                conv(15*384//self.num_slices, 8*384//self.num_slices, stride=1, kernel_size=3),
                nn.GELU(),
                conv(8*384//self.num_slices , 384//self.num_slices, stride=1, kernel_size=3),
            )
        ...
        self.maskedContextModel_mu = nn.Sequential(
                Mask3dAttention(dim = self.Mask_win_size*self.Mask_win_size*(384//num_slices), num_heads = 2)
            )

        self.maskedContextModel_sigma = nn.Sequential(
                Mask3dAttention(dim = self.Mask_win_size*self.Mask_win_size*(384//num_slices), num_heads = 2)
            )
    ...
    #     # print(y3_new_slices[0].shape)
    #     if firstOrder:
    #         firstOrder = torch.zeros(y0_new_slices[0].shape).to(inputs.device)
    ...
    #     out_H = H*2
    #     out_W = W*2
    #     output = torch.zeros(B,out_C,out_H,out_W).to('cuda')
    ...
    def forward(self, x):
        ...
        # [batch,num_H*num_W*num_C,window*window*C_split]
        # H = num_H * window_size
        # W = num_W * window_size
        # C = num_C * C_split
        means_output = self.maskedContextModel_mu(y_hat_zigzag)

        sliding_window = 3
        num_sliding = sliding_window**3
        token_dim = window_size*window_size*C//self.num_slices

        means_output_pad = F.pad(means_output, (0, 0, num_sliding, 0)).unsqueeze(3)
        ...
        unfold = nn.Unfold(kernel_size=(num_sliding, 1))
        ...
        means_input = torch.cat([means_output_unfold,means_zigzag_unfold],dim=2).view(-1,2*num_sliding*token_dim)
        scales_input = torch.cat([means_output_unfold,scales_zigzag_unfold],dim=2).view(-1,2*num_sliding*token_dim)


        means_input = means_input.view(-1,2*num_sliding*C//self.num_slices,window_size,window_size)
        scales_input = scales_input.view(-1,2*num_sliding*C//self.num_slices,window_size,window_size)
        # print(means_input.shape)

        mu = self.cc_mean_transforms(means_input).view(B,num_H*num_W*self.num_slices,384//self.num_slices,window_size,window_size)
        scale = self.cc_mean_transforms(scales_input).view(B,num_H*num_W*self.num_slices,384//self.num_slices,window_size,window_size)
        # print(mean.shape)
        ...
        # [batch,C,H,W]
        # means_smaple = latent_means.view(B,num_H*num_W*self.num_slices,window_size*window_size*C//self.num_slices)
        # means_smaple = means_smaple.repeat(1,num_H*num_W*self.num_slices,1).view(B,num_H*num_W*self.num_slices,num_H*num_W*self.num_slices,-1)
        # means_input = torch.cat([means_output,means_smaple], dim=2).view(B*num_H*num_W*self.num_slices,
        #                                                                 (num_H*num_W*self.num_slices+1)*C//self.num_slices,
        #                                                                 window_size,window_size)[]

        # scales_smaple = latent_scales.view(B,num_H*num_W*self.num_slices,window_size*window_size*C//self.num_slices)
        # scales_smaple = scales_smaple.repeat(1,num_H*num_W*self.num_slices,1).view(B,num_H*num_W*self.num_slices,num_H*num_W*self.num_slices,-1)
        # scales_input = torch.cat([means_output,scales_smaple], dim=2).view(B*num_H*num_W*self.num_slices,
        #                                                                 (num_H*num_W*self.num_slices+1)*C//self.num_slices,
        #                                                                 window_size,window_size)
        # scales_smaple = self.sampler(latent_scales)

        # means_input = torch.cat([means_smaple,context_sample], dim=0)
        # scales_input = torch.cat([scales_smaple,context_sample],dim=0)
    ...
    def compress(self, x):
        ...
        y_q_slice = self.gaussian_conditional.quantize(y_hat, "symbols", mu)
    ...
    def decompress(self, strings, shape):
        ...
        for i in range(num_H*num_W*self.num_slices):
            ...
            rv = torch.Tensor(rv).reshape(1, -1, H, W)
            y_hat = self.gaussian_conditional.dequantize(rv, mu)
            if i == 0:
                print('y_hat_slice:',y_hat[:,0:5,0:4,0:4])
                print('mu:',mu[:,0:5,0:4,0:4])
