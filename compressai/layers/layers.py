...
class ModelPromptedSwinTransformerBlock(SwinTransformerBlock):
    ...
    def forward(self, x, m, x_size):
        ...
        if self.shift_size > 0:
            shifted_x = torch.roll(x, shifts=(-self.shift_size, -self.shift_size), dims=(1, 2))
            shifted_m = torch.roll(m, shifts=(-self.shift_size // self.mask_down, -self.shift_size // self.mask_down),
                                   dims=(1, 2))
        else:
            shifted_x = x
            shifted_m = m
        ...
        x_windows = x_windows.view(-1, self.window_size * self.window_size, C)  # nW*B, window_size*window_size, C
        m_windows = m_windows.view(-1, (self.window_size // self.mask_down) * (self.window_size // self.mask_down),
        ...
        mask_b = torch.nn.functional.interpolate(mask_a[:, :,
                                                 (self.window_size // self.mask_down) ** 2:self.window_size ** 2 - (
                                                             self.window_size // self.mask_down) ** 2].unsqueeze(1), (
                                                 self.window_size ** 2,
                                                 (self.window_size // self.mask_down) ** 2)).squeeze(1)
        if self.mask_down > 1:
        ...
        # W-MSA/SW-MSA (to be compatible for testing on images whose shapes are the multiple of window size
        if self.input_resolution == x_size:
            attn_windows, attn_values = self.attn(x_windows,
                                                  mask=prompt_mask.to(x.device))  # nW*B, window_size*window_size, C
        else:
