...
def inference_entropy_estimation(model, x, context, filename, recon_path):
    ...
    grid_img2 = F.pad(
        grid_img2, (-padding_left, -padding_right, -padding_top, -padding_bottom)
    )
    #
    # grid_img3 = out_net["Teacher_output_features"]
    # grid_img4 = out_net["seg_hat"].view(1,-1,32,32)
    # grid_img5 = out_net["image_out"]
    grid_img3 = out_net["likelihoods"]['y'].view(1,-1,32,32)
    grid_img4 = out_net["likelihoods"]['z'].view(1, -1, 32, 32)

    # grid_img3 = F.pad(
    #     grid_img3, (-padding_left, -padding_right, -padding_top, -padding_bottom)
    # )
    # bit_allocation = -torch.log2(grid_img3)
    ...
    bitmap2 = (torch.log(grid_img3[0]) / (-math.log(2))).mean(dim=0).cpu().numpy()
    bitmap = (torch.log(grid_img4[0]) / (-math.log(2))).mean(dim=0).cpu().numpy()


    # Plot heatmap
    plt.figure(figsize=(6, 6))
    plt.imshow(bitmap2*1.5, cmap='jet', vmin=0, vmax=1)
    plt.colorbar(label='Bit Allocation')
    plt.title('Bit Allocation Heatmap')
    plt.axis('off')

    # Save to file
    plt.savefig(recon_path + '/heat_y_' + filename, bbox_inches='tight', dpi=300)

    # Optional: close the figure to free memory
    plt.close()

    plt.figure(figsize=(6, 6))
    plt.imshow(bitmap*1.5, cmap='jet', vmin=0, vmax=1)
    plt.colorbar(label='Bit Allocation')
    plt.title('Bit Allocation Heatmap')
    plt.axis('off')

    # Save to file
    plt.savefig(recon_path + '/heat_y2_' + filename, bbox_inches='tight', dpi=300)

    # Optional: close the figure to free memory
    plt.close()

    import numpy as np
    # If grid_img3 is a PyTorch tensor
    avg_img = grid_img3[0].mean(dim=0).cpu().numpy()  # shape: (32, 32)

    # 2D FFT
    fft2 = np.fft.fft2(avg_img)
    fft2_shifted = np.fft.fftshift(fft2)
    magnitude_spectrum = np.log1p(np.abs(fft2_shifted))
    ...
    plt.savefig(recon_path + '/FFT_y_' + filename, bbox_inches='tight', dpi=300)

    # Optional: close the figure to free memory
    plt.close()

    # If grid_img3 is a PyTorch tensor
    avg_img = grid_img4[0].mean(dim=0).cpu().numpy()  # shape: (32, 32)
    ...
    plt.savefig(recon_path + '/FFT_y2_' + filename, bbox_inches='tight', dpi=300)
    ...
    # reconstruct(grid_img - grid_img2, 'con_' + filename, recon_path)
    # grid_img2 = (grid_img2 - grid_img2.min()) / (grid_img2.max() - grid_img2.min())
    # reconstruct(grid_img2, 'res_'+filename, recon_path)
    # grid_img5 = (grid_img5 - grid_img2.min()) / (grid_img2.max() - grid_img2.min())
    # reconstruct(grid_img5, 'human_' + filename, recon_path)
...
def setup_args():
    ...
    parent_parser.add_argument("-r", "--recon_path", type=str, default="/home/exx/Documents/Tianma/ICM/save_model/deIMG8/", help="where to save recon img")
    parent_parser.add_argument(
        "-a",
        "--architecture",
        default='IC_seg_oj_small', # czigzag  IC_seg_oj_small
        type=str,
        choices=models.keys(),
    ...
    parent_parser.add_argument(
            ...
            default='/data/checkpoint/mask_faster_Human/1/4824.ckpt',
            # default='/home/exx/Downloads/stf_0035_best.pth.tar',
            dest="paths",
            type=str,
            nargs="*",
